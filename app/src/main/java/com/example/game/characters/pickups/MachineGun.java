...
public class MachineGun extends Item {
    ...
    public MachineGun(int ammo, int curAmmo, int ammoReload) {
        super(new Rectangle(0, 0, 0, 120, 90), 40, 1, 80, 0);

        image = MainActivity.machinegun;
    ...
    @Override
    public Bullet getNextBullet(Vec pos, float z, Vec dir) {
    ...
    @Override
    public Bullet getNextBullet(Vec pos, float z, Vec dir, Vec point, float targetZ) {
        if (countToCoolDown != 0 || curAmmo == 0) return null;
        countToCoolDown = 1;
        ...
        pos = pos.add(dir.multy(bulletR + 30));
